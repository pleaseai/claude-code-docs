...
<Steps>
  ...
  <Step title="Ask rules">
    Check `ask` rules from [settings.json](../settings.md#permission-settings). If an ask rule matches, the call falls through to your [`canUseTool` callback](./user-input.md) for confirmation, even in `bypassPermissions` mode.
...
<Warning>
  **Subagent inheritance:** Subagents inherit the parent session's permission mode. An [`AgentDefinition`'s `permissionMode`](./typescript.md#agentdefinition) can override it, except when the parent uses `bypassPermissions`, `acceptEdits`, or `auto`: those modes apply to every subagent and can't be overridden per subagent.
