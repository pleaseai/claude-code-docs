...
See [Session storage](./session-storage.md) for the full `SessionStore` interface and reference adapters.
...
* **Anthropic API**: the subprocess reads `ANTHROPIC_API_KEY` from its environment. Supply it from your secret manager, or set `ANTHROPIC_BASE_URL` to route model calls through a proxy that injects the key outside the container. See [Credential management](./secure-deployment.md#credential-management) for the proxy pattern and the [SDK overview](./overview.md#get-started) for supported authentication methods.
...
Large fanouts of concurrent [subagents](./subagents.md) from a single session can hit API rate limits. Break the work into smaller batches rather than issuing one wide dispatch.
...
For per-tenant network controls, see [Secure Deployment](./secure-deployment.md).
...
| No per-subagent wall-clock deadline                 | Cap each [subagent](./subagents.md) with `maxTurns` in its `AgentDefinition`. For background subagents only, `CLAUDE_ASYNC_AGENT_STALL_TIMEOUT_MS` sets a stall watchdog that fires when a `run_in_background` subagent stops producing output; it is not a total-runtime deadline. |
