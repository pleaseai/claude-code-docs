...
| `read_keys`               | No       | Array of `{id, key}`. Read-only: every `GET` endpoint, including listing caps, fetching one by ID, and reading [`/effective`](./claude-apps-gateway-spend-limits.md#effective) and [`/audit`](./claude-apps-gateway-spend-limits.md#audit).       |
